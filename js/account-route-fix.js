(() => {
  ...
  function normalize(route) { return VALID_ROUTES.has(route) ? route : null; }
  function appIsVisible() { const shell = document.getElementById('appShell'); return Boolean(shell && !shell.hidden); }
  function currentDomRoute() { return document.querySelector('.page-view.is-active[data-page]')?.dataset.page || null; }
  ...
  window.RebusRouter = { set: applyRoute, get: () => activeRoute, restore: () => applyRoute(activeRoute) };
